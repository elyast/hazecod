 ...
 * 
 * <p>
 * Before using this library, you must set the configuration file pattern with
 * {@link #setConfigFilePattern(String)} so that it will find your configuration
 * files.
 * </p>
 * 
 * <p>
 * The library finds all the configuration files matching the pattern. All the
 * defined beans that implement the {@link Keyword} interface are available as
 * keywords.
 * </p>
 */
public class SpringDocumentedLibrary extends
	DocumentedKeywordFactoryBasedLibrary<Keyword> {
    private String configFilePattern = null;

    public SpringDocumentedLibrary(String configFilePattern) {
	setConfigFilePattern(configFilePattern);
    }

    public SpringDocumentedLibrary() {
    }
	...
	KeywordsXmlBeanDefinitionReader reader = new KeywordsXmlBeanDefinitionReader(
		ctx, getClassLoader());
    ...
    /**
     * Sets the pattern. See class description for details. Must be set before
     * Robot calls {@link RobotJavaLibrary#runKeyword(String, Object[])} or
	...
	if (configFilePattern == null)
	    throw new IllegalStateException(
		    "Config file pattern must be set before calling getKeywordNames.");
    }
}
